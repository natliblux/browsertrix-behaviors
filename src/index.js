...
export class BehaviorManager {
	...
	init(opts = { autofetch: true, autoplay: true, autoscroll: true, siteSpecific: true }) {
		...
		if (!self.location.href.match(/.*www\.luxprivat\.lu.*anmeldung.html.*/))
		{
			console.log("Adding Autofetcher because URL is: " + self.location.href);
		...
		// Commented by Laszlo 13/09/2022 
		// TODO Only comment out if we are using a mainBehavior
	...
	async run(opts) {
		...
		// Added by Laszlo 13/09/2022 
		this.inited = false;
		this.init(opts);

		behaviorLog("*** Initialized behaviors for site: " + document.location.href);
